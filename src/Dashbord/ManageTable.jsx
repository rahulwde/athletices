  ...
  const handleDelete = (id) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        axios.delete(`http://localhost:5000/athletics/${id}`)
          .then(({ data }) => {
   ...
   <div className="overflow-x-auto p-5">
    <table className="min-w-full border-collapse border border-gray-300">
      ...
      <tbody>
        {event.map((task) => (
          <tr key={task._id}>
            <td className="border border-gray-300 p-2"><img src={task.image} alt="event" className="w-20 h-20 object-cover rounded" />
</td>
